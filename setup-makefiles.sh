...
cd ../../../
find "$OUTD/proprietary/" -type f > "$OUTD/proprietary-files.txt"
cd "$CWD"

while read line; do
    SOURCE="$line"
    TARGET=$(echo "$line" | sed -e 's/vendor\/'$VENDOR'\/'$DEVICE'\/proprietary/\/system/');
    echo "    $SOURCE:$TARGET \\" >> $BLOBMK
done < ../../../$OUTD/proprietary-files.txt
